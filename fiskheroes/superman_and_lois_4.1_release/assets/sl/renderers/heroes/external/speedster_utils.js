function init(renderer, trailType) {
    var ashake = renderer.bindProperty("fiskheroes:camera_shake").setCondition(entity => {
        var aspeed = entity.getData("fiskheroes:speed");
        ashake.factor = aspeed > 1 && entity.isSprinting() && entity.getData("fiskheroes:speed_sprinting") ? (Math.log(aspeed - 1) + 1) * 0.5 * Math.sin(Math.PI * entity.getInterpolatedData("fiskheroes:dyn/speed_sprint_timer")) : 0;
        return true;
    });
    ashake.intensity = 0.05;
