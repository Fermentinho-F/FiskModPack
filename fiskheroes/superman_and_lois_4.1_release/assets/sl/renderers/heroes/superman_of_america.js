...
loadTextures({
  "layer1": "sl:soa_2_layer1",
  "layer2": "sl:soa_2_layer2",
...
function init(renderer, trailType) {
  ...
  renderer.setTexture((entity, renderLayer) => {
    if ((renderLayer === "HELMET" && !(entity.getData("sl:dyn/suitup_timer") > 0.5)) || (renderLayer === "LEGGINGS" && !entity.getData("sl:dyn/suitup"))) {
      return "layer2";
    } else {
      return (entity.getData("sl:dyn/suitup") ? "null" : "layer1");
    }
  });
  ...
  renderer.setItemIcon("CHESTPLATE", "soa_2_1");
  renderer.setItemIcon("LEGGINGS", "lana_rho_2");
  renderer.setItemIcon("BOOTS", "lana_rho_3");
}
...
kutils.initEffects(renderer);
    ...
    boobas.setExtrude(0.60).setYOffset(1);
...
physics.setTickHandler(entity => {
  ...
  utils.bindTrail(renderer, "sl:soa_trail").setCondition(entity => entity.getData("fiskheroes:speeding") && entity.isSprinting() && !entity.getData("sl:dyn/suitup"));

  utils.bindTrail(renderer, "sl:suitlesstrail").setCondition(entity => entity.getData("fiskheroes:speeding") && entity.getData("sl:dyn/suitup"));
...
function initAnimations(renderer) {
  ...
  addAnimationWithData(renderer, "superman.GLITCH", "sl:bizarro_glitch", "sl:dyn/speedup_timer")
        .setData((entity, data) => {
