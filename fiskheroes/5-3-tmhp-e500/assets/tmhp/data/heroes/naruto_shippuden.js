...
function init(hero) {
    ...
    hero.setVersion("Naruto Shippuden");
    hero.setTier(5);
    ...
    hero.addKeyBind("CHARGED_BEAM", "Bijuu Dama", 2);
    hero.addKeyBind("UTILITY_BELT", "key.utilityBelt", 3);
    hero.addKeyBind("RAGE", "Rage Mod", 4);
    hero.addKeyBind("ONE_TAIL", "Four Tail Mod", 4);
...
function one_tailProfile(profile) {
    profile.inheritDefaults();
    profile.addAttribute("PUNCH_DAMAGE", 34.0, 0);
...
function isModifierEnabled(entity, modifier) {
    ...
    case "fiskheroes:damage_bonus":
        return modifier.id() == "oodama_rasengan" == (!entity.getData("tmhp:dyn/one_tailed"));
...
function getTierOverride(entity) {
   if (entity.getData("tmhp:dyn/one_tailed")) {
    ...
    return entity.getData("tmhp:dyn/rage") ? 3 : 2;
}