function create(renderer, icon, backBoosters) {
    ...
    var legs = renderer.createEffect("fiskheroes:booster").setIcon(icon);
    legs.setOffset(0.0, 7.0, 2.1).setSize(1.25, 3.0);
    ...
    if (backBoosters) {
        ...
        back.setOffset(3.0, 4.6, 1.75).setSize(1.25, 3.0);
        ...
        render: (entity, renderLayer, isFirstPersonArm, all) => {
            if (!isFirstPersonArm) {
                ...
                if (all || renderLayer == "BOOTS") {
                    ...
                    if (legs != null && entity.getData('fiskheroes:suit_open_timer') == 0 ) {
                        legs.progress = entity.getInterpolatedData("fiskheroes:dyn/booster_timer");
                        legs.speedScale = 0.5 * boost;
                        legs.setRotation(20 - 10 * boost, 0.0, 0 - 0 * f);
                        legs.flutter = 1 + boost;
                        legs.render();
                    }
                }
            }
