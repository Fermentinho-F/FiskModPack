...
loadTextures({
    ...
    "modok": "jmctheroes:modok",
    "lights": "jmctheroes:modok_lights"
});
...
function initEffects(renderer) {
    ...
    var model = renderer.createResource("MODEL", "jmctheroes:MODOK/Modok");
    ...
    var modelfull = renderer.createResource("MODEL", "jmctheroes:MODOK/ModokFull");
    ...
    var arm = utils.createModel(renderer, "jmctheroes:MODOK/ModokArm", "modok", null);
    arm.generateMirror();
...
function initAnimations(renderer) {
    ...
    addAnimation(renderer, "modok.SUIT", "jmctheroes:pose/modok").setData((entity, data) => {
        data.load(0, Math.max(entity.isAlive()));
        data.load(1, Math.sin((2*entity.loop(100))*Math.PI)/2);
        data.load(2, entity.getInterpolatedData("jmctheroes:dyn/moving_timer"));
    });
}
