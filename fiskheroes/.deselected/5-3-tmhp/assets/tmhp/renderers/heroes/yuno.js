...
loadTextures({
    "layer1": "tmhp:black_clover/yuno/prets_hooded",
    "layer2": "tmhp:black_clover/yuno/prets_layer2",
    "hood_open": "tmhp:black_clover/yuno/prets_layer1",
    "cape": "tmhp:black_clover/yuno/prets_cape",

    "wind_spirit": "tmhp:black_clover/yuno/bell",
    "spiritdive": "tmhp:black_clover/yuno/spiritdive",
    "crown": "tmhp:black_clover/yuno/spirit_crown",
    "wing": "tmhp:black_clover/yuno/wing",
...
function initEffects(renderer) {
    ...
    wing = spirit_wings.create(renderer, null, "wing", spirit_wings.SPIRITDIVE);
    ...
    crown.setModel(utils.createModel(renderer, "tmhp:black_clover/spirit_crown", null, "crown"));
    ...
    utils.bindBeam(renderer, "fiskheroes:charged_beam", "fiskheroes:charged_beam", "body", 0x00FF00, [
        { "firstPerson": [0.0, 6.0, -15.0], "offset": [0.0, 5.0, -25.0], "size": [20, 20] }
...
function initAnimations(renderer) {
    ...
    addAnimation(renderer, "basic.CHARGED_BEAM", "fiskheroes:aiming_left").setData((entity, data) => data.load(Math.max(entity.getInterpolatedData("fiskheroes:beam_charge") * 5 - 4, 0)));
...
function render(entity, renderLayer, isFirstPersonArm) {
    if (renderLayer == "CHESTPLATE") {
        ...
        grimoire.render();
        spiritdive.opacity = entity.getInterpolatedData("tmhp:dyn/spiritdive_timer");
        spiritdive.render();
        crown.opacity = entity.getInterpolatedData("tmhp:dyn/spiritdive_timer");
        crown.render();
        wind_spirit.opacity = !entity.getInterpolatedData("tmhp:dyn/spiritdive_timer");
        wind_spirit.render();
        spirit_of_zephyr.opacity = entity.getData("fiskheroes:shield");
        spirit_of_zephyr.render();
        cape.render(entity);
    }
    if (entity.getInterpolatedData("tmhp:dyn/spiritdive_timer")) {
        wing.render(entity, entity.getInterpolatedData("fiskheroes:shield_blocking_timer"));
    }
    if (!isFirstPersonArm) {
        shrink_lights.opacity = shrink_lights.progress = entity.getInterpolatedData("tmhp:dyn/spiritdive_timer");
        shrink_lights.progress /= Math.sqrt(entity.getData('tmhp:dyn/spiritdive_timer') > 0) * 2;
